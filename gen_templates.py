...
web_base = """<!-- {name_title} Template -->
<div class="resume-{name}" xmlns:th="http://www.thymeleaf.org">
    <style>
        .resume-{name} {{ font-family: {font}; padding: 32px; color: {color}; line-height: 1.5; min-height: 900px; font-size: 13px; background: {web_bg}; }}
        .resume-{name} .r-header {{ text-align: {header_align}; margin-bottom: 12px; }}
        .resume-{name} .r-name {{ font-size: {name_size}; font-weight: bold; text-transform: uppercase; margin-bottom: 6px; color: {color}; }}
        .resume-{name} .r-contact {{ font-size: 11.5px; color: {accent}; display: flex; justify-content: {header_align_flex}; flex-wrap: wrap; gap: 12px; }}
...
pdf_base = """<!DOCTYPE html>
<html xmlns:th="http://www.thymeleaf.org" lang="en">
<head><meta charset="UTF-8"/><title>Resume</title>
<style>
    @page {{ size: A4; margin: 15mm 18mm; }}
    body {{ font-family: {font}; margin: 0; padding: 0; color: {color}; line-height: 1.5; font-size: 11px; }}
    .r-header {{ text-align: {header_align}; margin-bottom: 10px; }}
    .r-name {{ font-size: {pdf_name_size}; font-weight: bold; text-transform: uppercase; margin-bottom: 4px; color: {color}; }}
    .r-contact {{ font-size: 10px; color: {accent}; text-align: {header_align}; margin-bottom: 10px; }}
...
import os
base_dir = r"d:\Downloads\Resume Builder\src\main\resources\templates"

for style_name, conf in styles.items():
    flex_align = 'center' if conf['header_align'] == 'center' else 'flex-start'
    
    web_content = web_base.format(
        name=style_name,
        name_title=style_name.capitalize(),
        font=conf['font'],
        color=conf['color'],
        accent=conf['accent'],
        header_align=conf['header_align'],
        header_align_flex=flex_align,
        name_size=conf['name_size'],
        border=conf['border'],
        web_bg=conf['web_bg']
    )
    
    pdf_name_size = str(int(conf['name_size'].replace('px', '')) - 4) + 'px'
    
    pdf_content = pdf_base.format(
        font=conf['font'],
        color=conf['color'],
        accent=conf['accent'],
        header_align=conf['header_align'],
        pdf_name_size=pdf_name_size,
        border=conf['border']
    )
    
    # Write Web
    web_path = os.path.join(base_dir, 'templates', f"{style_name}.html")
    with open(web_path, 'w', encoding='utf-8') as f:
        f.write(web_content)
        
    # Write PDF
    pdf_path = os.path.join(base_dir, 'pdf', f"{style_name}.html")
    with open(pdf_path, 'w', encoding='utf-8') as f:
        f.write(pdf_content)

print("Generated all 14 files.")
